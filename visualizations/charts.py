...
─────────────────────────
All Plotly figures used by the dashboard.

Design principles
-----------------
• Dark-glass aesthetic — transparent backgrounds, subtle grid lines
• Every method is a pure function: in → Figure, no side effects
• A single LAYOUT_BASE dict ensures visual consistency across all charts
"""
...
PALETTE = {
    ...
    "surface": "rgba(0,0,0,0)",        # transparent — Streamlit sets the bg
}
...
class WeatherCharts:
    ...
    def temperature_trend_chart(
        ...
        """
        Line chart of historical temperature with optional min/max band
        and an ML-predicted future segment.
        """
        fig = go.Figure()
    ...
    def multi_city_comparison(self, city_data: List[Dict]) -> go.Figure:
        """
        Side-by-side bar charts: temperature and humidity across cities.
        """
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=[c.replace("1)", "0.7)") for c in colors],
    ...
    def wind_rose(self, df: pd.DataFrame) -> go.Figure:
        """Polar bar chart of average wind speed per compass direction."""
    ...
    def temperature_heatmap(self, df: pd.DataFrame) -> go.Figure:
        """2-D heatmap: hour of day (x) vs calendar date (y) → temperature."""
        if df.empty:
            return go.Figure().update_layout(**LAYOUT_BASE, title="🔥 Temperature Heatmap (no data)")
