...
──────────────────────
Business-logic layer: analytics, forecasting, anomaly detection.

Every method is stateless and side-effect-free — pass in DataFrames,
get back DataFrames or primitive results.  No I/O happens here.
"""
...
class WeatherAnalytics:
    ...
    def comfort_level(self, temp: float, humidity: float) -> str:
        """Return a human-readable comfort label with emoji."""
    ...
    def temperature_trend(df: pd.DataFrame, window: int = 6) -> str:
        """
        Fit a linear slope over the last *window* records.

        Returns a directional label: "📈 Rising" / "📉 Falling" / "➡️ Stable".
        """
        if len(df) < 3:
    ...
    def forecast_temperature(
        ...
        """
        Simple linear extrapolation of temperature over historical data.

        Returns a DataFrame with columns [timestamp, predicted_temp],
        ...
        steps       = max(1, hours_ahead // 3)   # 3-hour intervals
    ...
    def detect_anomalies(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Flag unusual readings with Isolation Forest.

        Adds a boolean *is_anomaly* column to the returned DataFrame.
        Requires at least 10 records; returns the frame unchanged otherwise.
        """
        if len(df) < 10:
    ...
    def correlation_matrix(df: pd.DataFrame) -> pd.DataFrame:
        """Return a Pearson correlation matrix for key weather variables."""
    ...
    def daily_summary(df: pd.DataFrame) -> pd.DataFrame:
        """
        Aggregate per-record data into daily statistics.
        Works on both history DataFrames and forecast DataFrames.
        """
        if df.empty:
            return pd.DataFrame()
        out = df.copy()
    ...
    def get_insights(self, current: Dict, history_df: pd.DataFrame) -> List[str]:
        """
        Generate a short list of human-readable insight strings
        based on current conditions and recent history.
        """
        insights: List[str] = []
