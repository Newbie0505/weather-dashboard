...
try:
    ...
    st.info("Add your OPENWEATHER_API_KEY to the .env file and restart.")
    st.stop()
...
# ── KPI Cards ─────────────────────────────────────────────────────────────────
def kpi(label, value, u=""):
    return (f"<div class='kpi-card'>"
            f"<div class='kpi-label'>{label}</div>"
            f"<div class='kpi-value'>{value}</div>"
            f"<div class='kpi-unit'>{u}</div></div>")
...
with tab_temp:
    ...
    with c1:
        st.plotly_chart(charts.temperature_trend_chart(history_df, pred_df), use_container_width=True)
    with c2:
    ...
    i3.info(f"🌬️ Wind dir  {get_wind_direction(current.get('wind_deg', 0))} ({current.get('wind_deg', 0):.0f}°)")
...
with tab_compare:
    ...
    if compare_data:
        ...
        st.warning("Could not load comparison data. Check city names in the sidebar.")
...
with tab_analytics:
    ...
    if not history_df.empty:
        ...
        if not found.empty:
            st.warning(f"⚠️ {len(found)} anomalous reading(s) detected.")
...
with tab_history:
    ...
    with hdr:
        st.markdown(
            f"<div class='section-hdr'>Records for {primary_city} — last {history_days} days</div>",
            unsafe_allow_html=True,
        )
    with export_col:
        if not history_df.empty:
    ...
    if not history_df.empty:
        ...
        show_cols = [c for c in ["timestamp", "temperature", "feels_like", "humidity",
                                  "pressure", "wind_speed", "cloudiness", "weather_desc"]
        ...
        st.info(f"No history for {primary_city} yet. Every visit saves a new record automatically.")